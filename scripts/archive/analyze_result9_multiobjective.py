...
SPRING_LITE_DIR = ROOT_DIR / "spring_lite"
if str(SPRING_LITE_DIR) not in sys.path:
    sys.path.insert(0, str(SPRING_LITE_DIR))
...
def flatten_run(run_root: Path, shards: int = 16) -> Optional[Dict[str, object]]:
    ...
    # Re-read Tx_Details so duplicate and missing transaction hashes are checked
    # even for runs finalized before those fields were added to analysis.json.
    latency = analyze_tx_latency_details(run_root / "expTest" / "result")
    ...
    expected_start_tx = 2_944_019 if identity["window"] == "test2M" else 2_500_000
    ...
    unique_tx_hash_count = int(latency.get("unique_tx_hash_count", 0) or 0)
    duplicate_tx_hash_count = int(latency.get("duplicate_tx_hash_count", 0) or 0)
    missing_tx_hash_count = int(latency.get("missing_tx_hash_count", 0) or 0)
    valid_latency_row_ratio = (
        valid_latency_count / latency_row_count if latency_row_count else 0.0
    )
    latency_coverage_ratio = valid_latency_count / expected_total
    ...
    row: Dict[str, object] = {
        ...
        **identity,
        ...
        "latency_row_count": latency_row_count,
        "valid_latency_row_ratio": valid_latency_row_ratio,
        # Retain the old field name for consumers, but make its meaning explicit.
        "valid_latency_ratio": valid_latency_row_ratio,
        "latency_coverage_ratio": latency_coverage_ratio,
        "unique_tx_hash_count": unique_tx_hash_count,
    ...
    window_config_ok = (
        int(config.get("TotalDataSize", 0) or 0) == expected_total
        and int(config.get("DatasetStartTx", -1)) == expected_start_tx
    )
    row["protocol_ok"] = (
        row["state_check"] == "passed"
        and math.isclose(effective_total, expected_total, rel_tol=0.0, abs_tol=1e-6)
        and latency_row_count == expected_total
        and valid_latency_count == expected_total
        and unique_tx_hash_count == expected_total
        and duplicate_tx_hash_count == 0
        and missing_tx_hash_count == 0
        and window_config_ok
    )
    return row
...
def group_rows(rows: Iterable[Dict[str, object]]) -> List[Dict[str, object]]:
    ...
    for row in rows:
        # Failed runs remain in the per-run audit artifact, but never
        # contribute to formal group means or comparison tables.
        if not bool(row.get("protocol_ok")):
            continue
...
def build_report(rows: List[Dict[str, object]], groups: List[Dict[str, object]]) -> str:
    ...
    lines = [
        ...
        "- 未通过 protocol_ok 的运行只保留在逐运行诊断中，不进入均值和正式对照表。",
...
def run_analysis(results_root: Path, output_dir: Path) -> Dict[str, object]:
    ...
    return {
        ...
        "rejected_run_count": sum(not bool(row["protocol_ok"]) for row in rows),
